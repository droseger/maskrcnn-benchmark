...
from maskrcnn_benchmark.engine.plot_maps import plot
...
def inf(args, cfg):
    ...
    save_dir = os.path.join(cfg.OUTPUT_DIR, "valInf")
    mkdir(save_dir)
    ...
    data_loaders_val = make_data_loader(cfg, mode=DatasetMode.VAL_AP, is_distributed=distributed)
    output_tuple = {}
...
def get_model_paths(directory):
    ...
    return [join(directory, file) for file in files if ".pth" in file]
